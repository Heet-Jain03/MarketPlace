...
async def execute(
    ...
    if req.context and "research_context" in req.context:
        ...
        research_context = (
            ...
            f"Key Findings: {'; '.join(rc.get('key_findings',[]))}"
        )

    system_prompt = (
        f"You are WriteBot, an expert content writer. "
        f"Write a {style_desc(req.style)} {req.content_type} of {length_map.get(req.length,'600-800 words')}. "
        f"Return JSON with keys: title, content, word_count (integer), meta_description."
    )
