...
async def execute(
    req: ExecuteRequest,
    # Accept EITHER a user JWT (direct call) OR an internal X-Agent-Token (orchestrator)
    authorization:        Optional[str] = Header(None),
    x_agent_token:        Optional[str] = Header(None),
    x_openrouter_api_key: Optional[str] = Header(None, alias="X-OpenRouter-API-Key"),
):
    # Auth: orchestrator uses X-Agent-Token (internal); browser uses Bearer JWT
    if x_agent_token:
        verify_internal_agent_token(AGENT_TOKEN, x_agent_token)
    elif authorization:
        verify_user_jwt(authorization)   # validates JWT, raises 401 if invalid
    else:
    ...
    system_prompt = (
        ...
        f"Return JSON with keys: title, summary, key_findings (array of strings), raw_text."
    )
