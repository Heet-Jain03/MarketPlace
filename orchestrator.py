...
@app.post("/auth/buy")
async def proxy_buy(req: dict, authorization: str = Header(...)):
    async with httpx.AsyncClient() as client:
        resp = await client.post(
            "http://127.0.0.1:8004/auth/buy",
            json=req,
...
@app.post("/auth/login")
async def proxy_login(req: dict):
    async with httpx.AsyncClient() as client:
        resp = await client.post("http://127.0.0.1:8004/auth/login", json=req)
        return resp.json() if resp.status_code == 200 else {"error": resp.text}

@app.post("/auth/signup")
async def proxy_signup(req: dict):
    async with httpx.AsyncClient() as client:
        resp = await client.post("http://127.0.0.1:8004/auth/signup", json=req)
        return resp.json() if resp.status_code == 200 else {"error": resp.text}

@app.post("/auth/refresh")
async def proxy_refresh(req: dict):
    async with httpx.AsyncClient() as client:
        resp = await client.post(
            "http://127.0.0.1:8004/auth/refresh",
            json=req
        )
        return resp.json() if resp.status_code == 200 else {"error": resp.text}
