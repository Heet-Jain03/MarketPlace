...
@app.get("/a2a/execute")
async def execute_get(data_or_topic: str = None):
    ...
    if not data_or_topic:
        ...
        <textarea id="data">AI market trends</textarea><br>
        ...
        async function run() {
            ...
            try {
                const res = await fetch(`?data_or_topic=${encodeURIComponent(data)}`);
                const json = await res.json();

                const result = json.result || {};
                const summary = result.executive_summary || "No summary available";

                const insights = (result.key_insights || []).map(i => `<li>${i}</li>`).join("");
                const recs = (result.recommendations || []).map(i => `<li>${i}</li>`).join("");

                document.getElementById("output").innerHTML =
                    `<h3>Summary</h3><p>${summary}</p>
                    <h4>Insights</h4><ul>${insights}</ul>
                    <h4>Recommendations</h4><ul>${recs}</ul>`;

            } catch (e) {
                document.getElementById("output").innerHTML =
                    "<p style='color:red;'>Error fetching result</p>";
    }
}
        </script>
    ...
    req = ExecuteRequest(data_or_topic=data_or_topic)
